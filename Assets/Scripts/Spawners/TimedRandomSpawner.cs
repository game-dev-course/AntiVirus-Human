...
public class TimedRandomSpawner: MonoBehaviour {
    ...
    [Tooltip("Time that will take until the object will be automatically destroyed")] [SerializeField]
    ...
    IEnumerator SpawnRoutine() {    // co-routines
        ...
        while (true) {
            ...
            StartCoroutine(DestructCooldown(newObject));
        }
    }

    IEnumerator DestructCooldown(GameObject gameObject)
    {
        yield return new WaitForSeconds(destructionTime); // co-routines
        Destroy(gameObject);
    }
}